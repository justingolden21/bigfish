...
function showSnackbar(message, type) {
	...
	snackbar_queue++;
	setTimeout( ()=> { createSnackbar(message, type); }, SNACKBAR_TIME_TOTAL*(snackbar_queue-1) );
}
function createSnackbar(message, type) {
	...
	if(type=='achievement' || type=='unlock') $('#snackbar').addClass('important');
	else $('#snackbar').removeClass('important');

	// note: removed 'error' from list
	if(['success', 'achievement', 'unlock'].includes(type) )playSoundEffect(type);
	...
	if(notification_history.length > MAX_HISTORY_SIZE) notification_history.shift();
