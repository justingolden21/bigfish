...
let sprites_img, sprites_loaded = false;
...
function updateNumDrawnFish() {
	...
	for(let type of types) {
		...
		else if(num_drawn_fish[type] < inventory.fish[type] && num_drawn_fish[type] < MAX_DRAWN_FISH) {
			let amt = Math.min(inventory.fish[type], MAX_DRAWN_FISH) - num_drawn_fish[type];
...
function drawSeaweed() {
	for(let i=0; i<seaweedX.length; i++) {
		...
		ctx.drawImage(sprites_img, (seaweedFrames[i]*SPRITE_SIZE), SPRITE_SIZE*2, SPRITE_SIZE, SPRITE_SIZE,
			seaweedX[i], MAX_Y, SPRITE_SIZE, SPRITE_SIZE);
		if(Math.random()>0.95) {
			seaweedFrames[i]++;
...
const FISH_SPEEDS = { // 4,3,2 and 2,3,4 are other ideas
	small: 3,
	medium: 3,
	big: 3,
};
...
class DrawnFish {
	constructor(type) {
		...
		this.y = random(0, canvas.height - SPRITE_SIZE*FISH_SCALES[this.type]);
	...
	move() { // move according to speed, random direction
		...
		this.facing_left = Math.random() >= 0.01 ? this.facing_left : !this.facing_left;
		
		if(!this.facing_left)
			this.x += FISH_SPEEDS[this.type]; // + random(-1,1);
		else
			this.x -= FISH_SPEEDS[this.type]; // + random(-1,1);
		if(Math.random() >= 0.1) {
			if(this.y <=0) {
	...
	draw() { // draws fish on canvas
		drawFish(this.x, this.y, this.facing_left, this.frame, this.type, this.color, this.species, this.special);
	}
	update() {
		this.move();
		this.nextFrame();
...
function drawFish(x, y, facing_left, frame, type, color, species, special) {
	...
	ctx.filter = `hue-rotate(${color + special_num * 180}deg) invert(${special_num})`; // note: performance hit
	ctx.drawImage(sprites_img,
		(frame+species*NUM_FRAMES)*SPRITE_SIZE, direction*SPRITE_SIZE, SPRITE_SIZE, SPRITE_SIZE,
		x, y, SPRITE_SIZE*scale, scale*SPRITE_SIZE);
}
