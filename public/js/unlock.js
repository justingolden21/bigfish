...
let achievements = {
	...
	"There's an Achievement for That!?": [
		'Have 1,000,000,000,000,000 big fish',
		-1,
		true,
	],
...
function checkUnlocks(inventory) {
	...
	for (let type of types) {
		if (
			inventory.fish[type] >= 200 ||
			(inventory.fish[type] >= 40 && aquariums_used >= 0.75)
		) {
	...
	if (inventory.fish.big >= 1e15)
		checkAchievement("There's an Achievement for That!?");

	if (total_hatcheries >= 1) checkAchievement('Financially Responsible');
	if (
		inventory.buildings.small_hatchery >= 1 &&
		inventory.buildings.medium_hatchery >= 1 &&
		inventory.buildings.big_hatchery >= 1
	)
	...
	let fish_sold =
		stats.fish.small.sold + stats.fish.medium.sold + stats.fish.big.sold;
	if (fish_sold >= 1e3)
		checkAchievement('So Long and Thanks for all the Fish');
...
function checkAchievement(achievement_name) {
	if (achievements[achievement_name][COMPLETED] == -1) {
		...
		showSnackbar(
			`Achievement Unlocked: ${achievement_name}<br><small>${achievements[achievement_name][DESCRIPTION]}</small>`,
			'achievement'
		);

		if (allAchievementsComplete()) {
			openModal(
				'<i class="fas fa-crown"></i> Congrats!',
				`<p class="text-center">You completed the game in <b>${secToStr(
					stats.ticks
				)}</b></p>
...
function getAchievementHTML() {
	...
	for (let name in achievements) {
		if (achievements[name][COMPLETED] != -1) {
			tmpHTML += `<p><i class="fas fa-award"></i> <b>${name}</b> : ${
				achievements[name][DESCRIPTION]
			} ( ${secToStr(achievements[name][COMPLETED])} )</p>`;
		}
	}
	tmpHTML += '<hr>';
	for (let name in achievements) {
...
const getCompletedAchievementSize = () =>
	Object.keys(achievements).filter((a) => achievements[a][COMPLETED] != -1)
		.length;

const allAchievementsComplete = () =>
	getCompletedAchievementSize() == getAchievementSize();
