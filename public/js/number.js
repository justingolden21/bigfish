...
function prettyPrintNum(num) {
	...
	return addZeros( (round(num/Math.pow(10, num_mag*3) )*multiplier).toString() ) + ' ' + num_abrev[num_mag-1];
}

const addZeros = (str) => str.indexOf('.') == -1 ? str : str.split('.')[0] + '.' + str.split('.')[1].padEnd(3, '0');

const round = (num, places=3) => Math.round(num * Math.pow(10,places) ) / Math.pow(10,places);

// https://stackoverflow.com/a/28203456/4907950
const numDigits = num => Math.max(Math.floor(Math.log10(Math.abs(num) ) ), 0) + 1;
