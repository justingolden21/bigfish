...
function displayTick(inventory, hungry, rates, VALS) {
	...
	displayNum('.num-fish', inventory.fish.small+inventory.fish.medium+inventory.fish.big);
	...
	$('#v-pills-aquarium .info-toggle-card .card-body .progress').html( getProgressBar(space_used/space_total) );

	if(unlocks['big-fish']) { // multibar
		let sm_space = VALS.space.fish.small * inventory.fish.small / space_total;
	...
	if($('.modal-title').html().indexOf('Stats')!=-1 && $('.modal').hasClass('show') ) {
		updateStatsDisplay();
	}
	...
	if($('.modal-title').html().indexOf('Insights')!=-1 && $('.modal').hasClass('show') ) {
		updateInsightsDisplay(inventory);
	}

	// if multiplier modal is open
	if($('.modal-title').html().indexOf('Multiplier')!=-1 && $('.modal').hasClass('show') ) {
