const getIncreasingCost = (amount, step, start) => step * (getTriangular(amount+start) - getTriangular(start) );

const maxCanAffordIncreasingCost = (coins, step, start) =>
	Math.floor(Math.sqrt( (start+0.5)*(start+0.5) + 2*coins/step) - (start+0.5) );
